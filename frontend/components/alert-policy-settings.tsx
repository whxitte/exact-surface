...
import { BellRing, Save, RotateCcw, ChevronDown } from "lucide-react";
...
export function AlertPolicySettings({ programId }: { programId?: string }) {
  ...
  const [isOpen, setIsOpen] = useState(!programId);
  ...
  return (
    <Card>
      <CardContent className="p-5">
        ...
        <div
          className={cn(
            isCollapsible && "grid transition-[grid-template-rows] duration-300 ease-in-out"
          )}
          style={isCollapsible ? { gridTemplateRows: isOpen ? "1fr" : "0fr" } : undefined}
        >
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                {family(
                  ...
                  <label className="flex flex-col gap-1 text-xs text-muted-foreground">
                    Only these ports (seperated by comma or specify a range, blank = any)
                    <Input
                      value={v.port_filter}
