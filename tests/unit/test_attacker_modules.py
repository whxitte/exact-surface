...
# --------------------------------------------------------------------------- #
# JS mining — regressions from the first real-world run (example.com)
# --------------------------------------------------------------------------- #
def test_script_paths_are_not_reported_as_endpoints():
    """`/js/admin.1a2b3c4d.js` is another bundle, not an admin endpoint. The first
    real run reported it hundreds of times tagged ADMIN, which drowned the signal."""
    out = js_miner.mine('var a="/js/admin.1a2b3c4d.js";', "https://x.com/app.js")
...
def test_property_chains_are_not_reported_as_hostnames():
    ...
    `array.prototype.find` and `deep.nested.chain` look like hosts."""
    js = '"array.prototype.find";"object.entries";"deep.nested.chain";'
